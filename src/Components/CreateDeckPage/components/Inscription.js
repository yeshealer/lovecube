...
export default function Inscription(props) {
    ...
    const handleNextOccasion = () => {
        if (occasion.length <= 0) {
            ...
            setInscription(inscription)
            navigate(`/create-deck/${pathname}/describe`)
        }
    }
    ...
    return (
        <div className='flex flex-col items-start w-9/10 sm:w-2/3 md:w-1/2'>
            ...
            <div className='text-2xl md:text-3xl my-8'>Choose an inscription</div>
            ...
            <div className='w-full flex flex-col items-center'>
                ...
                <div class="flex mt-2 w-full">
                    <input type="text" id="website-admin" value={inscription} onChange={e => handleSetInscription(e)} class={`rounded-none outline-none rounded-l-lg bg-gray-50 border text-gray-900 ${isRequired ? 'focus:ring-rose-300 border-rose-700 border-2' : 'focus:border-[#3e9ca3]'} block flex-1 min-w-0 w-full text-sm border-gray-300 p-2.5 focus:ring-green-400 focus:ring-2`} placeholder="" />
